...
const creator = (item) =>{
    let number =0; //this doesn't work as a toggler
    return (
        <CreatorStyle key={item.name} className={item.name}>
        <h2>{item.name}</h2>
        <button onClick={() =>{number+=1;console.log(number)}}>▄</button>
        {number===0 && <div> {/* i'm still figuring out how to make toggle working */}
